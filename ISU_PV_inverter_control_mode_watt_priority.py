# -*- coding: utf-8 -*-
"""
This is the smart inverter control mode with watt priority 
"""
#%%
import numpy as np
...
def constant_pf_mode(category, variable, parameter=None):
    ...
    Q_inj_PF = np.tan(np.arccos(power_factor)) * P
    Q_abs_PF = -Q_inj_PF
    
    # Determine final Q_inj and Q_abs
    Q_inj = min(Q_inj_PF, Q_inj_std)
    Q_abs = max(Q_abs_PF, Q_abs_std)
    
    return np.array([Q_inj, Q_abs, Q_inj_PF, Q_abs_PF, Q_inj_std, Q_abs_std]), P
...
def active_reactive_mode(category, variable, parameter):
    ...
    # Determine final Q_inj and Q_abs
    Q_inj = min(Q_inj_APRP,Q_inj_std)
    Q_abs = max(Q_abs_APRP,Q_abs_std)

    return np.array([Q_inj,Q_abs,Q_inj_APRP,Q_abs_APRP,Q_inj_std,Q_abs_std]), P
...
def constant_reactive_mode(category, variable, *parameter):
    ...
    Q_inj = min(Q_inj_RP, Q_inj_std)
    Q_abs = max(Q_abs_RP, Q_abs_std)
...
def vol_var_mode(category, variable, parameter):
    ...
    Q_VV = 0
    if V <= VL:  
        raise ValueError("The lowest voltage is 0.9.")
    elif V <= V1:
        Q_VV = Q1
    elif V <= V2:
        ...
        raise ValueError("The highest voltage is 1.1.")

    # Q
    if Q_VV >= 0:
        Q = min(Q_VV, Q_inj_std)
    else:
        Q = max(Q_VV, Q_abs_std)
...
def reactive_power_generation(mode_control, PV_Gen_P, voltage, PV_installed):
    ...
    if mode_control == 'without control':
        ...
        if PV_Gen_P < 0.05 * PV_installed:
            ...
            PV_nameplate = PV_installed / 0.9
            PV_Gen_Q, PV_Gen_P_actual = control_mode('constant power factor', 'B', [PV_Gen_P, voltage], [PV_nameplate, power_factor, PV_installed])
        Q_gen = PV_Gen_Q[1]
        ...
        if PV_Gen_P < 0.05 * PV_installed:
            ...
            PV_Gen_Q, PV_Gen_P_actual = control_mode('voltage-reactive power', 'B', [PV_Gen_P, voltage], parameter)
        Q_gen = PV_Gen_Q[0]
        P_gen = PV_Gen_P_actual
        ...
        P_min = 0
        P_min_s = 0
        nameplate_apparent_power_rating = PV_installed / 0.9
